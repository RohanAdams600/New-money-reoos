...
function listenToTrack({ track, source, deepgram, elevenlabs, getSnapshot, onEvent }) {
  let socket;
  let closed = false;
  ...
  (async () => {
    try {
      ...
      socket.on('message', async (message) => {
        ...
        try {
          const reply = await hudBrain.answer(transcript, getSnapshot());
          onEvent({ type: 'reply', text: reply });
          activityLog.log(`🎙️ Asked "${transcript}" — answered: ${reply}`);
...
async function ensureAgent({ roomName, getSnapshot, onEvent = () => {} }) {
  ...
  room.on(RoomEvent.TrackSubscribed, (track) => {
    ...
    stopFns.push(listenToTrack({ track, source, deepgram, elevenlabs, getSnapshot, onEvent }));
  });
